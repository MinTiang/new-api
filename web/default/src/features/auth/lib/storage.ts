...
export function getRegistrationCode(): string {
  if (typeof window === 'undefined') return ''
  try {
    return window.localStorage.getItem(STORAGE_KEYS.REGISTRATION_CODE) ?? ''
  } catch (error) {
    // eslint-disable-next-line no-console
...
export function saveRegistrationCode(code: string): void {
  if (typeof window === 'undefined') return
  try {
    window.localStorage.setItem(STORAGE_KEYS.REGISTRATION_CODE, code)
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('Failed to save registration code:', error)
  }
}

export function removeRegistrationCode(): void {
  if (typeof window === 'undefined') return
  try {
    window.localStorage.removeItem(STORAGE_KEYS.REGISTRATION_CODE)
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error('Failed to remove registration code:', error)
  }
}
